...
window.DMP_CONTEXT={__version__:'5.7.18',contexts:{},contextsByName:{},lastContext:null,bundleFunctions:{},logEnabled:null,set:function(version,contextid,data,templates){DMP_CONTEXT.logEnabled=DMP_CONTEXT.logEnabled||data.__router__.log;if(DMP_CONTEXT.__version__!=version){DMP_CONTEXT.log(['Framework version on server is',version,' but dmp-common.js is',DMP_CONTEXT.__version__,'- unexpected behavior may occur.']);}
...
return ret;},loadBundle(template_functions){var templates=Object.keys(template_functions);DMP_CONTEXT.log(['Linking bundle with',templates.length,'functions'],templates);for(var i=0;i<templates.length;i++){DMP_CONTEXT.bundleFunctions[templates[i]]=template_functions[templates[i]];}},checkBundleLoaded(contextid){var context=DMP_CONTEXT.contexts[contextid];if(typeof context==="undefined"){return;}
...
context.pendingCalls++;DMP_CONTEXT.checkBundleLoaded(contextid);},log(messages,data){if(DMP_CONTEXT.logEnabled){console.info('[DMP] '+messages.join(' '),data||'');}},};})()