...
window.DMP_CONTEXT={__version__:'5.7.7',contexts:{},contextsByName:{},lastContext:null,bundleFunctions:{},set:function(version,contextid,data,templates){if(DMP_CONTEXT.__version__!=version){console.warn('DMP framework version is '+version+', while dmp-common.js is '+DMP_CONTEXT.__version__+'. Unexpected behavior may occur.');}
...
while(context.triggerCount>0){context.triggerCount--;for(var i=0;i<context.templates.length;i++){var template=context.templates[i];if(DMP_CONTEXT.bundleFunctions[template]){DMP_CONTEXT.bundleFunctions[template]();}}}},triggerBundleContext(contextid){var context=DMP_CONTEXT.contexts[contextid];if(typeof context==="undefined"){return;}
